...
class Solo {
    constructor(tipo) {
        ...
        this.numRachaduras = 0; // Inicializa o número de rachaduras
    }
    ...
    aumentarErosao() {
        ...
        this.erosao += taxa;
        this.altura -= taxa;
        this.numRachaduras += 0.1; // Aumenta o número de rachaduras gradualmente
    }
    ...
    mostrar() {
        ...
        // Desenha as rachaduras no solo
        stroke(100, 50, 0); // Cor marrom escura para as rachaduras
        strokeWeight(1);
        for (let i = 0; i < this.numRachaduras; i++) {
            let x = random(width);
            let y = this.altura + random(height - this.altura);
            line(x, y, x + random(-10, 10), y + random(-10, 10)); // Linhas pequenas para representar as rachaduras
        }
